...
#define ENCODING_ROOT 0
static const int32_t candidate_shifts[21]={10,8,8,16,14,8,7,8,8,8,13,16,12,0,6,7,8,8,6,5,7};
...
static void shared_frontend(const radar_feature21_golden_point_t *p,uint32_t n,
  ...
  if(ENCODING_ROOT)encoding_root_row(out,out);
  for(int j=0;j<21;j++){
...
void test_round(const int64_t *x,uint32_t n,int32_t *y) {
  for(uint32_t i=0;i<n;i++) y[i]=qmlp_round_shift_even_i64(x[i],16);
}
