...
def main():
    ...
    for split in ['train','val']:
        ...
        arrays={name:np.lib.format.open_memmap(d/(name+'.npy'),mode='w+',dtype=np.int8,shape=((n*2 if name=='synthetic' else small if name=='natural' or name.startswith('sparse_') else n),13)) for name in names}
        if split=='train':oldarrays=ex.Pools().arrays;oldmap=dict(zip(names,oldarrays))
        else:oldmap={c:ex.validation(c)[0] for c in names}
            ...
            for j,row in enumerate(rows):
                ...
                if len(p)>=3:
                    ...
                    for name,z in variants.items():
                        x=native(z);assert np.array_equal(x[:23],oldmap[name][i]),(split,name,i);arrays[name][i]=x[23:]
                    if split=='train':
                        ...
                        for k in [1,2]:
                            ...
                            assert np.array_equal(x[:23],oldmap['synthetic'][ix]);arrays['synthetic'][ix]=x[23:]
                    ...
                    for name,z in variants.items():
                        x=native(z);assert np.array_equal(x[:23],oldmap[name][si]),(split,name,si);arrays[name][si]=x[23:]
                    si+=1
        ...
        reports.append(dict(split=split,sequences=seqcount,kept=n,sparse=si,views=names,feature_rows=sum(len(a) for a in arrays.values()),base23_mismatches=0,label_mismatches=0,future_history_violations=0))
    assert time.monotonic()-start<1800;dump(LOG/'preparation.json',dict(status='PASS',splits=reports,wall_seconds=time.monotonic()-start,source_sha256=source,scope='Same causal train/development observations; no new test data; all base23 features checked, new13 descriptors computed from point data',output_bytes=size_guard()))
