"""Thirty-three fixed-budget downward/upward representation trajectories."""
from dim_common import *

def main():
    assert json.loads((LOG/'training_preflight.json').read_text())['status']=='PASS'
    assert json.loads((LOG/'reference_replay.json').read_text())['status']=='PASS'
    ...
    sources=[p for p in HERE.iterdir() if p.name in ['plan.json','dim_common.py','dimension_features.c','preflight.py','prepare.py','training_preflight.py','train.py','evaluate.py','build_package.py','gates.py'] or p.suffix=='.md']
    sources += [ex.HERE/'ex_common.py',ex.HERE/'features.c',ex.HERE/'evidence_manifest.json',LOG/'dependency_manifest.json',ab.HERE/'ab_common.py',ab.HERE/'train.py',sw.HERE/'pools.py',qc.HERE/'common.py',qc.HERE/'plan.json',MECH/'data/scales.json',LOG/'training_preflight.json',OLD/'data/matching.json']
    sources += [p for p in (LOG/'data').rglob('*.npy')]
    sources += [LOG/'training_preflight/calibration.npz',LOG/'training_preflight/low_count_properties.json',LOG/'reference_replay.json',LOG/'preparation.json',LOG/'synthetic_preflight.json',MECH/'data/train/labels.npy',MECH/'data/val/labels.npy',OLD/'data/train/natural_labels.npy',OLD/'data/train/synthetic_labels.npy']
