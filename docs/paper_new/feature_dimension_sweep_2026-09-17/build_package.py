...
def main():
    ...
    main_src=(ROOT/'docs/paper_new/conditional_statistics_2026-09-16/inference_main.c').read_text().replace('shared_frontend(p,n,CANDIDATE_MODE,candidate_shifts,features);','int8_t full[36]; enrichment(p,n,full); for(int j=0;j<RADAR_MLP_INPUT_DIM;j++)features[j]=full[input_positions[j]];').replace('features[21]','features[RADAR_MLP_INPUT_DIM]').replace('i<21','i<RADAR_MLP_INPUT_DIM')
    ...
    for tag in VARIANTS:
        ...
        for seed in SEEDS:
            ...
            (d/'candidate.c').write_text(native_source(tag)+network+positions+main_src+batch)
    ...
    for tag in REFERENCES:
        for seed in SEEDS:
            source=ENR/'package'/(tag+'_seed'+str(seed));destination=out/'frozen_references'/source.name
            shutil.copytree(source,destination)
            for f in source.iterdir():
                if f.is_file():assert sha(f)==sha(destination/f.name)
            reused.append(dict(tag=tag,seed=seed,source=str(source.relative_to(ROOT)),sha256={f.name:sha(f) for f in source.iterdir() if f.is_file()}))
    dump(out/'reused_references.json',reused)
