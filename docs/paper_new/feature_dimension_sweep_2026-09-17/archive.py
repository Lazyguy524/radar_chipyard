...
MIRROR=Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/论文写作/双向维度实验与收敛_2026-09-17')
...
def main():
    ...
    archive=ARCHIVE/'feature_dimension_sweep_results.tar.gz';assert not archive.exists()
    ...
    for p in sorted(HERE.rglob('*')):
        ...
        if p.suffix=='.md':
            target.write_text(p.read_text().replace('../../../logs/feature_dimension_sweep_20260917/','证据/'))
    ...
    for p in [ARCHIVE/'feature_dimension_sweep_before.bundle',ARCHIVE/'feature_dimension_sweep_after.bundle',archive]:
        shutil.copyfile(p,backup/p.name);assert sha(p)==sha(backup/p.name)
    md='项目/毕业论文——研究生/Chipyard论文知识库/论文写作/双向维度实验与收敛_2026-09-17'
    block='\n\n<!-- feature-dimension-sweep-20260917 -->\n2026-09-17：[['+md+'/README|双向维度实验与收敛]]：完成8～36维11组×三种子的固定33条新训练，复用六个旧16/23模型；比较紧凑表示、点数支持编码、分位数分布与等维冗余对照，公开全部分组和失败结果；结果与取舍见一页结论。软件开发选型证据，独立泛化与板级成本另列，不新增硬件工作。\n'
    for index in [Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/Chipyard雷达SoC论文知识库.md'),Path('/home/soooarr/obsidian/Codex/Chipyard/Chipyard Index.md')]:
        s=index.read_text();assert 'feature-dimension-sweep-20260917' not in s;index.write_text(s+block)
