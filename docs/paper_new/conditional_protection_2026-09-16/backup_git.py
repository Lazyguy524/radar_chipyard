...
REF='refs/heads/backup/radar-thesis-20260916-protection'
PACKAGES=['feature_approximation_pilot','qmlp_int8_convergence','conditional_statistics','mechanism_convergence','moment_encoding','conditional_protection']
...
def main():
    ...
    for package in PACKAGES:
        d=ROOT/'docs/paper_new'/(package+'_2026-09-16')
        ...
        paths.update(p for p in l.rglob('*') if p.is_file() and not p.is_symlink() and '__pycache__' not in p.parts and p.suffix in {'.py','.c','.h','.scala','.json','.pt','.npz'}
                     and not any(x in p.parts for x in ['raw','data','prepared']) and not p.name.startswith('git_backup_'))
    paths.update(ROOT/p for p in ['docs/paper_new/README.md','docs/paper_new/00_project_requirements_and_memory.md','tests/radar-feature21-qmlp-cpu-fullchain-profile.c'])
    ...
    paths.add(manifest)
    with tempfile.TemporaryDirectory(prefix='radar_protection_git_') as temporary:
        env=dict(os.environ,GIT_INDEX_FILE=str(Path(temporary)/'snapshot.index'))
        subprocess.run(['git','read-tree',old or head],cwd=ROOT,env=env,check=True)
        entries=[]
        ...
        message='radar thesis: '+('freeze conditional protection plan and prior research sources' if a.phase=='before' else 'archive bounded protection results and design reflection')+'\n\nSelective backup branch; original work branch and staged index preserved. Large raw data are separately indexed, not included.\n'
        commit=git(['commit-tree',tree,'-p',old or head],input=message,text=True).strip()
    ...
    bundle=Path('/home/soooarr')/'radar_thesis_backups_20260916';bundle.mkdir(exist_ok=True)
    bundle_path=bundle/('conditional_protection_'+a.phase+'.bundle')
    ...
    try:
        push=subprocess.run(['git','push','origin',REF+':'+REF],cwd=ROOT,env=env,text=True,capture_output=True,timeout=90)
    ...
    except subprocess.TimeoutExpired as e:status=dict(remote_verified=False,error='Remote operation timed out',command=str(e.cmd))
