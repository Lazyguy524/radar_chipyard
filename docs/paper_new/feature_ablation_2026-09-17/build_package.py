...
def main():
    ...
    main_src=(ROOT/'docs/paper_new/conditional_statistics_2026-09-16/inference_main.c').read_text().replace('shared_frontend(p,n,CANDIDATE_MODE,candidate_shifts,features);','subset_frontend(p,n,features);').replace('features[21]','features[RADAR_MLP_INPUT_DIM]').replace('i<21','i<RADAR_MLP_INPUT_DIM')
    ...
    for tag in VARIANTS:
        ...
        for seed in SEEDS:
            ...
            (d/'candidate.c').write_text(frontend_source(tag)+network+main_src+batch)
            ...
            for condition in ['clean','sparse_single','sparse_context']:
                ...
                expected=sw.validation(condition)[0][:,cols];want=reference(expected)[-1];x=np.empty((n,dim),np.int8);logits=np.empty((n,2),np.int32)
            ...
            ex,fr=base.scales();dump(d/'contract.json',dict(tag=tag,seed=seed,columns=cols,shifts=(ex+fr)[0,cols].tolist(),input='1..511 points, little endian int16 [x,y,compensated radial velocity,RCS], Q8.8; upstream same-sequence/sensor/GT-track history; sparse observations do not update history',history=PLAN['data']['history'],output='class0 labels7/8; class1 labels0/1/2/3; tie chooses0',scope='Software candidate reference; no board measurement or deployment promotion',integer_weights='all INT8, biases INT32; ties-to-even and ReLU saturation127'))
