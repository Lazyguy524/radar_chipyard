...
MIRROR=Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/论文写作/特征必要性与删组重训_2026-09-17')
...
def main():
    ...
    archive=ARCHIVE/'feature_ablation_results.tar.gz';assert not archive.exists()
    ...
    for p in sorted(HERE.rglob('*')):
        ...
        if p.suffix=='.md':
            target.write_text(p.read_text().replace('../../../logs/feature_ablation_20260917/','证据/'))
    ...
    for p in [ARCHIVE/'feature_ablation_before.bundle',ARCHIVE/'feature_ablation_after.bundle',archive]:
        shutil.copyfile(p,backup/p.name);assert sha(p)==sha(backup/p.name)
    md='项目/毕业论文——研究生/Chipyard论文知识库/论文写作/特征必要性与删组重训_2026-09-17'
    block='\n\n<!-- feature-ablation-20260917 -->\n2026-09-17：[['+md+'/README|特征必要性与删组重训]]：完成21/20/18/16维×三种子的固定12条删组重训、13条件/分组审查与整数精确去重诊断；结果与取舍见一页结论。软件开发选型证据，独立泛化与板级成本另列，不新增硬件工作。\n'
    for index in [Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/Chipyard雷达SoC论文知识库.md'),Path('/home/soooarr/obsidian/Codex/Chipyard/Chipyard Index.md')]:
        s=index.read_text();assert 'feature-ablation-20260917' not in s;index.write_text(s+block)
