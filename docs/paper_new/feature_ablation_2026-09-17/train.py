...
def main():
    ...
    sources=[p for p in HERE.iterdir() if p.suffix in ('.py','.json','.md','.c')]
    sources += [sw.HERE/'train.py',sw.HERE/'pools.py',qc.HERE/'common.py',qc.HERE/'plan.json',MECH/'data/scales.json',LOG/'preflight.json',OLD/'data/matching.json']
    sources += [p for p in (OLD/'data/train').iterdir() if p.suffix=='.npy']
    sources += [MECH/'data/train/proxy.npy',MECH/'raw/train/proxy/uniform_quarter_0.npy',MECH/'raw/train/proxy/central_half.npy']
    ...
    data=pools.Pools('proxy');weights=torch.tensor(data.class_weights,dtype=torch.float64);target=torch.tensor(data.y)
    vy=np.load(MECH/'data/val/labels.npy')
    for tag_base in VARIANTS:
        cols=keep(tag_base);mu=data.mu[cols];std=data.std[cols];x=data.x[:,cols]
        norm=torch.tensor((x-mu)/std,dtype=torch.float32);raw=torch.tensor(x,dtype=torch.float64)
        vx=np.load(MECH/'data/val/proxy.npy')[:,cols]
        source=1
        for seed in SEEDS:
                ...
                for epoch in range(1,61):
                    ...
                    if epoch in cfg['checkpoints']:
                        ...
                        export(snap/'export',q.export(),cols)
                ...
                if tag_base=='full21':
                    ...
                    reproduction='EXACT_OLD_PROXY_NATURAL'
                report=dict(variant=tag_base,columns=cols,seed=seed,tag=tag,validation=metric,initialization_sha256=init,schedule_sha256=schedules.hexdigest(),
                    full21_reproduction=reproduction,fold_error=err,integer_layer_mismatches=mismatch,qat_buffer_sha256=state,wall_seconds=time.monotonic()-began,curve=curve)
