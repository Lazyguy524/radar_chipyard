...
MIRROR=Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/论文写作/可解释特征补充与维度扩展_2026-09-17')
...
def main():
    ...
    archive=ARCHIVE/'feature_enrichment_results.tar.gz';assert not archive.exists()
    ...
    for p in sorted(HERE.rglob('*')):
        ...
        if p.suffix=='.md':
            target.write_text(p.read_text().replace('../../../logs/feature_enrichment_20260917/','证据/'))
    ...
    for p in [ARCHIVE/'feature_enrichment_before.bundle',ARCHIVE/'feature_enrichment_after.bundle',archive]:
        shutil.copyfile(p,backup/p.name);assert sha(p)==sha(backup/p.name)
    md='项目/毕业论文——研究生/Chipyard论文知识库/论文写作/可解释特征补充与维度扩展_2026-09-17'
    block='\n\n<!-- feature-enrichment-20260917 -->\n2026-09-17：[['+md+'/README|可解释特征补充与维度扩展]]：完成16/19/20/23维五组×三种子的固定15条训练；新增几何及速度/RCS分布描述，包含等维冗余对照、全条件分组与可解释反例；结果与取舍见一页结论。软件开发选型证据，独立泛化与板级成本另列，不新增硬件工作。\n'
    for index in [Path('/home/soooarr/obsidian/项目/毕业论文——研究生/Chipyard论文知识库/Chipyard雷达SoC论文知识库.md'),Path('/home/soooarr/obsidian/Codex/Chipyard/Chipyard Index.md')]:
        s=index.read_text();assert 'feature-enrichment-20260917' not in s;index.write_text(s+block)
