...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--stage',choices=['encoding'],required=True);args=parser.parse_args();stage=args.stage
    ...
    config=PLAN[stage];cfg=qc.PLAN['training'];start=time.monotonic();out=LOG/'training';out.mkdir(exist_ok=False)
    ...
    source_paths=[HERE/'plan.json',HERE/'train_encoding.py',HERE/'common_encoding.py',qc.HERE/'common_encoding.py',qc.HERE/'plan.json',LOG/'data/scales.json']
    ...
    for mode in config['representations']:
        ...
        for seed in config['seeds']:
            ...
            for epoch in range(1,config['qat_epochs']+1):
                ...
                if epoch in PLAN['encoding']['snapshots']:
                    ...
                    params['input']=dict(encoding=mode,representation='centered_moments',flags=PLAN['encoding']['flags'][mode],statistics_precision=[24,24],feature_slots=21,
                        scale_source='logs/moment_encoding_20260916/data/scales.json',runtime_normalization=False)
