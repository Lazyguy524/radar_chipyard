...
def main():
    ...
    config=PLAN[stage];cfg=qc.PLAN['training'];start=time.monotonic();out=LOG/stage;out.mkdir(exist_ok=False)
    ...
    source_paths=[HERE/'plan.json',HERE/'train_fixed.py',HERE/'common.py',qc.HERE/'common.py',qc.HERE/'plan.json',LOG/'data/scales.json']
        ...
        for seed in config['seeds']:
            ...
            for epoch in range(1,config['qat_epochs']+1):
                ...
                if epoch in PLAN['factorial']['snapshots']:
                    logits=predict_tensor(qat,vraw,True).astype(np.int32);val=qc.metric(vy,logits.argmax(1));row['validation']=val
