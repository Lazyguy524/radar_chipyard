...
static long compare_buffers_once(const char *tag)
{
  volatile uint32_t *tx = tx_buffer();
  volatile uint32_t *rx = rx_buffer();
...
static void evict_dcache_lines(void)
{
  ...
  printf("Eviction sweep complete, accum=0x%08x\n", accum);
}
...
int main(void)
{
  ...
  dump_channel_regs("MM2S final", MM2S_DMACR, MM2S_DMASR, MM2S_SA, MM2S_SA_MSB, MM2S_LENGTH);
  dump_channel_regs("S2MM final", S2MM_DMACR, S2MM_DMASR, S2MM_DA, S2MM_DA_MSB, S2MM_LENGTH);

  riscv_fence_rw_rw();

  long first_bad = compare_buffers_once("Before eviction");
  evict_dcache_lines();
  riscv_fence_rw_rw();
  long second_bad = compare_buffers_once("After eviction");
